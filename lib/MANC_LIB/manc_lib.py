...
def dekoder_manchester(signal): # Fungsi untuk mendekode sinyal Manchester
    ...
    for i in range(0, len(signal), 2):  # Iterasi setiap dua level sinyal
        ...
        if pair == [-1, 1]: # Jika pasangan adalah [-1, 1]
            ...
            binary += '0' # Tambahkan '1' ke string biner
...
def plot_manchester(input_text): # Fungsi untuk memplot sinyal Manchester
    ...
    for i in range(total_bits): # Tambahkan teks untuk setiap bit pada grafik input biner
        axs[0].text(i * bit_width + bit_width / 2, 1.1, binary_input[i], ha='center', fontsize=9, fontweight='bold') # Teks untuk setiap bit input biner
    ...
    axs[1].set_title("Hasil Manchester Enkoder", fontweight='bold') # Judul grafik sinyal Manchester
    ...
    axs[2].set_title("Hasil Dekoder", fontweight='bold') # Judul grafik sinyal biner yang didekode
    ...
    for i in range(len(binary_decoded)):   # Tambahkan teks untuk setiap bit pada grafik sinyal biner yang didekode
        axs[2].text(i * bit_width + bit_width /2, 1.1, binary_decoded[i], ha='center', fontsize=9, fontweight='bold') # Teks untuk setiap bit pada grafik sinyal biner yang didekode

    plt.tight_layout(rect=[0, 0.03, 1, 0.84])  # Mengatur tata letak agar tidak ada tumpang tindih
