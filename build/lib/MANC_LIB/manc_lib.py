...
def ascii_to_bin(text): # Fungsi untuk mengonversi teks ke biner
    return ''.join(format(ord(c), '08b') for c in text) # Fungsi ini mengonversi setiap karakter ke representasi biner 8-bit

def bin_to_ascii(binary_str): # Fungsi untuk mengonversi string biner ke teks ASCII
...
def enkoder_manchester(text):
    binary = ascii_to_bin(text) # Mengonversi teks ke biner
...
def dekoder_manchester(signal): # Fungsi untuk mendekode sinyal Manchester
    ...
    return bin_to_ascii(binary), binary # Mengonversi biner ke teks ASCII
...
def plot_manchester(input_text): # Fungsi untuk memplot sinyal Manchester
    ...
    fig.text(0.52, 0.85, f"Input Binary: {binary_input}", ha='center', fontsize=12) # Teks biner input
    fig.text(0.52, 0.80, f"Decoded Text: {decoded_text}", ha='center', fontsize=12) # Teks yang didekode dari sinyal Manchester

    # Plot input binary
    axs[0].plot(time_input, volt_input, drawstyle='steps-post', color='green', linewidth=2) # Plot sinyal biner input
    axs[0].set_title("Sinyal Input") # Judul grafik input biner
    ...
    axs[1].set_title("Hasil Manchester Enkoder") # Judul grafik sinyal Manchester
    ...
    axs[2].set_title("Hasil Dekoder") # Judul grafik sinyal biner yang didekode
...
def main_manchester(input_txt):

    input_text = input_txt.strip()  # Menghapus spasi di awal dan akhir teks input
    if not input_text:  # Memeriksa apakah input kosong
        raise ValueError("Input tidak boleh kosong")  # Tampilkan pesan kesalahan jika input kosong
    plot_manchester(input_text) # Panggil fungsi untuk memplot sinyal Manchester


print("Program Enkoding dan Dekoding Manchester\n")  # Tampilkan judul program
print(f"="*50)  # Tampilkan garis pemisah
text = input("Masukkan karakter atau teks untuk di enkoding : ")  # Input teks dari pengguna
main_manchester(text)  # Panggil fungsi utama dengan teks input
